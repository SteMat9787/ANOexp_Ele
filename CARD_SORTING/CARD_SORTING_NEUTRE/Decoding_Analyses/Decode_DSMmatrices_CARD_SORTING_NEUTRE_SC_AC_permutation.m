...
study_path= fileparts(cd); %the previous folder in the hierarchy if path
 ...
 numVoxelsTokeep=190;%this is the number of entries in each DSMvec when a stimuli are incuded
 
    %%prepare the targets (as for 2 categories)
    targets=repmat(1:2,1,20)'; %2 are the gropu and 19 the subject in each group
...
chunks=[1:20,1:20]';
    ...
    ds.sa.chunks=chunks;
     ...
     load (strcat(study_path,'/DSMs_vectors.mat'));

     for i=1:20
         AC(i,:)=mat2gray(DSM_AC_all(i,:));
     end
     pecora=1;
     for i=[1:20]
         SC(pecora,:)=mat2gray(DSM_SC_all(i,:));
         pecora=pecora+1;
     end
     %
...
%open this to save
save(strcat('Results_perm_CARD_SORTING_NEUTRE/Perm_SC_AC',num2str(niter),'_40sub_ALL_words'),'real_accuracy','perm_accuracy');