...
%set the standard deviation limit to exclude outliers (when this is an high
%number e.g. 10 you are probably plotting all the data)
limit=2.5;
...
if Gr==1
Colors=repmat(Col_A,3,1)
elseif Gr==2
    Colors=repmat(Col_B,3,1)
end
...
for i_col=1:length(XTick)
    col= data(:,i_col);
         ...
         %exclude outliers
        threshold = limit * std( col );
        validRange = mean( col ) + [-1 1] * threshold;
        col = col( col >= validRange(1) & col <= validRange(2) );
